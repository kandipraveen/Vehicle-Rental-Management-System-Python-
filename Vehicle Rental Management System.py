...
class Vehicle:
    ...
    def return_vehicle(self):
        if not self.is_available:
            ...
            print(f"⚠️ Vehicle {self.vehicle_id} was not rented.")
    ...
    def return_vehicle(self, vehicle_id):
        for v in self.rented_vehicles:
            if v.vehicle_id == vehicle_id:
                ...
                print(f"✅ {self.name} returned {v.brand} {v.model}.")
                return
        print(f"⚠️ Vehicle ID {vehicle_id} not found in {self.name}'s rented list.")
...
print("🔢 Total vehicles rented so far:", Vehicle.get_total_rented())

# Rental cost calculation using @property
days = 3
print(f"💰 Estimated cost for {days} days (Car): {car1.rental_cost(days)}")
